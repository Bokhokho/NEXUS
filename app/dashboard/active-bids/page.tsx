...
/* ✅ NEW: date normalization helper for consistent sorting/filtering */
const toISO = (v: unknown): string => {
  ...
  // Try MM/DD/YYYY or DD/MM/YYYY (guess by first number)
...
type Bid = {
  ...
  setAside: string;
  dueDate: string;
  /* ✅ NEW: canonical date for sorting/filtering */
  dueDateISO: string;
  dueTime: string;
  dueTimeMA: string;
  location: string;
...
export default function ActiveBidsPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("activeBidsVisibleColumns");
  ...
  const [filters, setFilters] = useState<Filters>(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("bidFilters");
      return stored
        ? JSON.parse(stored)
        : {
            prime: "",
  ...
  /* ✅ NEW: applied filters (Search button commits these) */
  const [appliedFilters, setAppliedFilters] = useState<Filters>(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("bidAppliedFilters");
      return stored
        ? JSON.parse(stored)
        : {
            prime: "",
...
useEffect(() => {
  ...
  const handleSave = () => {
    /* ✅ ensure ISO stays in sync with display date */
  ...
  const handleImportExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    sheet.eachRow((row, index) => {
      ...
      /* ✅ NEW: compute ISO date for sorting/filtering */
  ...
  // 🔍 Filtering now uses **appliedFilters** (committed via Search)
  const filteredBids = useMemo(() => {
    return bids.filter((b) =>
      Object.entries(appliedFilters).every(([key, value]) => {
        if (!value) return true;
        /* ✅ NEW: date filter goes against ISO key */
        if (key === "dueDate") {
  ...
  const columns = [
    ...
    /* ✅ NEW: sort by dueDateISO, display dueDate (fallback to ISO) */
    {
      key: "dueDate",
      header: "Due Date",
  ...
  const clearFilters = () => {
    ...
    setFilters(empty);
    /* ✅ also clear applied filters so table resets immediately */
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-muted-foreground">
            Track and manage all ongoing bid opportunities
          </p>
        </div>
